...
namespace DXApplication1.DAL
{
    public class BUS_Item
    {
        ...
        public object GetFoodByIDCategory(int IDCategory)
        {
            try
            {
                SE_08 db = new SE_08();
                if (IDCategory == 0)
                {
                    return db.Items.Select(p => new { ID = p.Id, Name = p.Name, Price = p.Price, Name_Category = p.Category.Name }).ToList();
                }
                else
                {
                    return db.Items.Where(p => p.Id_Category == IDCategory).Select(p => new { Id = p.Id, Name = p.Name, Price = p.Price, Name_Category = p.Category.Name }).ToList();
                }
