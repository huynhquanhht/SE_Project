...
namespace DXApplication1.DAL
{
    public class BUS_Employee
    {
        ...
        public List<CbbItem> GetListEmployeeDontHaveAccount()
        {
            try
            {
                SE_08 db = new SE_08();
                var listAccount = db.Accounts.Select(p => p).ToList();
                var listEmployee = db.Employees.Select(p => p).ToList();
                List<CbbItem> list = new List<CbbItem>();
               
                foreach (var j in listEmployee)
                {
                    bool exist = false;
                    foreach (var i in listAccount)
                    {
                        if (i.Id_Employee == j.Id)
                        {
                            exist = true;
                            break;
                        }
                    }
                    if (exist == false) list.Add(new CbbItem((int)j.Id, (String)j.Name));
